...
export default function SingleComment({commmentText, commentRate, commentId, getAllComment}){
        ...
        <Col xs={12} style={{marginBlock: 5, borderTop: "1px solid black", paddingTop: "10px"}} className="d-flex justify-content-between align-items-start" >
         <Col xs={10}>
           <div className="d-flex justify-content-between mb-2">
               <span className={`m-0 fw-bolder ${dark ? "dark-mode" : ""}`}>Rating:</span><p className={`m-0 fw-bolder ${dark ? "text-light" : ""}`}>{commentRate}</p>
           </div>
           <div>
               <span className={`mb-3 fw-bolder ${dark ? "dark-mode" : ""}`}>Recensione:</span>
               <p style={{minHeight: 50, wordBreak: "break-word", maxHeight: 100, border: "1px solid lightgrey", overflow: "auto"}} className={`my-3 ps-2 ${dark ? "text-light" : ""}`}>{commmentText}</p>
           </div>
         </Col>
